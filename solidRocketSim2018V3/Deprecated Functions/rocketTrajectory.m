function [] = rocketTrajectory( engineType,Theta0,mp)
...
t = zeros(1,tsim);
t(1:lf) = f(1,:);
Fx = zeros(1,tsim);     %total in x direction
Fy = zeros(1,tsim);     %total in y direction
Ft = zeros(1,tsim);     %thrust
Ft(1:lf) = f(2,:); 
W = zeros(1,tsim);      %gravitational force
Fd = zeros(1,tsim);     %drag
...
x(1) = 0;
y(1) = 0;
Rho(1) = 1.225;          %Sea level air density
Mass(1) = fullMass;
